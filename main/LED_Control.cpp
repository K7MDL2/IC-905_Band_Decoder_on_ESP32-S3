...
#include "LED_Control.h"
    ...
    void ledc_init(void)
    {
        ...
        for (int i=0; i< 8; i++) {
            ...
            if (i==7)  // The green LED has almos 1V less voltage drop than the blue so has more current and is brighter, dim it.
                ESP_ERROR_CHECK(ledc_set_duty(LEDC_MODE, (ledc_channel_t) i, LEDC_ON_DUTY/4));//  Bands PTT, Bands 1-6 and PwrOn
            else
        ...
        for (int i=0; i< 7; i++) {  // Leave the Pwr_On LED lit.
